// ✅ Contracts.jsx بعد التحويل لاستخدام SweetAlert2
...
const Contracts = () => {
  ...
  const handleDelete = async (id) => {
    confirm({
      title: 'هل أنت متأكد من حذف هذا العقد؟',
    ...
    <div className="pt-4 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        <div className="flex gap-3">
          <button onClick={exportToPDF} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md flex items-center gap-2 shadow">
            <Download size={16} /> <span>تصدير PDF</span>
          </button>
          <button onClick={exportToExcel} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md flex items-center gap-2 shadow">
        ...
        <Link to="/contracts/new" className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2 rounded-md flex items-center justify-center gap-2 shadow">
          <Plus size={18} />
          <span>عقد جديد</span>
        </Link>
      </div>
                ...
                <tr key={contract._id} className="hover:bg-gray-50 text-center transition duration-200">
                  ...
                  <td className="px-3 py-2 border">
                    <div className="flex justify-center flex-wrap gap-2 rtl:flex-row-reverse">
                      <Link to={`/contracts/${contract._id}`} className="flex items-center gap-1 text-blue-600 hover:text-blue-800" title="تفاصيل">
                        <Eye size={18} />
                        <span className="hidden sm:inline text-xs">تفاصيل</span>
                      </Link>

                      <Link to={`/contracts/edit/${contract._id}`} className="flex items-center gap-1 text-yellow-600 hover:text-yellow-800" title="تعديل">
                        <Pencil size={18} />
                        <span className="hidden sm:inline text-xs">تعديل</span>
                      </Link>

                      {['admin'].includes(role) && (
                        <button onClick={() => handleDelete(contract._id)} className="flex items-center gap-1 text-red-600 hover:text-red-800" title="حذف">
                          <Trash size={18} />
                          <span className="hidden sm:inline text-xs">حذف</span>
                        </button>
                      )}

                      <Link to={`/invoices/contracts/${contract._id}`} className="flex items-center gap-1 text-green-600 hover:text-green-800" title="الفاتورة">
                        <FileText size={18} />
